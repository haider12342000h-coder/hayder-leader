function ProfileSidebarCard({ healthSummary, alerts }) {
  return (
    <div className="space-y-5 xl:sticky xl:top-28">
      <section className="overflow-hidden rounded-[30px] border border-white/80 bg-white/95 p-5 shadow-soft backdrop-blur-xl">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-800">موثوق</span>
        </div>

        <div className="mt-4 rounded-[24px] bg-gradient-to-br from-brand-50 via-white to-cyan-50 p-4">
          <p className="text-sm leading-8 text-slate-700">{healthSummary.text}</p>
        </div>
      ...
      <section className="rounded-[30px] border border-rose-200 bg-rose-50/80 p-5 shadow-soft">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-rose-700">أولوية</span>
        </div>

        <div className="mt-4 space-y-3">
          {alerts.map((alert) => (
            <div key={alert} className="flex items-start gap-3 rounded-2xl bg-white px-4 py-3 text-sm font-semibold text-slate-900">
